...
# CCR-block auto-strip (2026-07-17): every `ccr start` RE-INJECTS two managed blocks into
# ~/.codex/config.toml (a "# BEGIN CCR managed profile" block and a provider block routing
# codex to http://127.0.0.1:3456) — once ccr stops, bare `codex` fails with "stream
# disconnected before completion". Reviews run bare codex, so claude-sakana strips these on
# exit (finally). Balanced BEGIN/END pairs are removed wholesale via non-greedy regex (an
# unmatched BEGIN is left intact — no data loss); a markerless re-injection is caught by the
# [model_providers.claude-code-router] table + model_provider-line fallbacks.
function Remove-CcrCodexBlocks {
    param([string]$Path = (Join-Path $HOME '.codex\config.toml'))
    if (-not (Test-Path $Path)) { return }
    $text = Get-Content $Path -Raw
    $new = $text -replace '(?ms)^[ \t]*#[ \t]*BEGIN CCR managed.*?^[ \t]*#[ \t]*END CCR managed[^\r\n]*\r?\n?', ''
    $new = $new -replace '(?ms)^\[model_providers\.claude-code-router\][^\[]*', ''
    $new = $new -replace '(?m)^[ \t]*model_provider[ \t]*=[ \t]*"claude-code-router"[^\r\n]*\r?\n?', ''
    if ($new -ne $text) { Set-Content -Path $Path -Value $new -Encoding utf8NoBOM -NoNewline }
}
